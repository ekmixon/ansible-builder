...
class AnsibleBuilder:
    ...
    def build_command(self):
        ...
        for key, value in self.build_args.items():
            build_arg = f"--build-arg={key}={value}" if value else f"--build-arg={key}"
...
class BaseDefinition:
    ...
    @property
    def version(self):
        if version := self.raw.get('version'):
            return str(version)
        else:
    ...
    @property
    def ansible_config(self):
        if ansible_config := self.raw.get('ansible_config'):
...
class UserDefinition(BaseDefinition):
    def __init__(self, filename):
        ...
        if self.raw.get('dependencies') is not None and not isinstance(
            self.raw.get('dependencies'), dict
        ):
            raise DefinitionError(textwrap.dedent(
                f"""
    ...
    def get_dep_abs_path(self, entry):
        ...
        """
        if req_file := self.raw.get('dependencies', {}).get(entry):
            return (
                req_file
                if os.path.isabs(req_file)
                else os.path.join(self.reference_path, req_file)
            )
    ...
    def validate(self):
        ...
        if invalid_keys := yaml_keys - set(ALLOWED_KEYS):
            raise DefinitionError(textwrap.dedent(
                f"""
        ...
        if self.raw.get('dependencies') is not None:
            ...
            if invalid_dependencies_keys := dependencies_keys - set(
                CONTEXT_FILES.keys()
            ):
                raise DefinitionError(textwrap.dedent(
                    f"""
        ...
        if additional_cmds := self.get_additional_commands():
            ...
            if unexpected_keys := set(additional_cmds.keys()) - expected_keys:
                raise DefinitionError(
                    f"Keys {*unexpected_keys,} are not allowed in 'additional_build_steps'."
                )

        if ansible_config_path := self.raw.get('ansible_config'):
...
class Containerfile:
    ...
    def __init__(self, definition,
        ...
        self.steps = [
            f"ARG EE_BASE_IMAGE={self.definition.build_arg_defaults['EE_BASE_IMAGE']}",
            f"ARG EE_BUILDER_IMAGE={self.definition.build_arg_defaults['EE_BUILDER_IMAGE']}",
        ]
    ...
    def prepare_ansible_config_file(self):
        if ansible_config_file_path := self.definition.ansible_config:
    ...
    def prepare_prepended_steps(self):
        if additional_prepend_steps := self.definition.get_additional_commands():
            if prepended_steps := additional_prepend_steps.get('prepend'):
                return self.steps.extend(AdditionalBuildSteps(prepended_steps))

        return False

    def prepare_appended_steps(self):
        if additional_append_steps := self.definition.get_additional_commands():
            if appended_steps := additional_append_steps.get('append'):
    ...
    def prepare_introspect_assemble_steps(self):
        ...
        if any(self.definition.get_dep_abs_path(thing) for thing in ('galaxy', 'system', 'python')):
            ...
            if requirements_file_exists := os.path.exists(
                os.path.join(self.build_outputs_dir, CONTEXT_FILES['python'])
            ):
            ...
            if bindep_exists := os.path.exists(
                os.path.join(self.build_outputs_dir, CONTEXT_FILES['system'])
            ):
    ...
    def prepare_galaxy_stage_steps(self):
        self.steps.extend(
            [
                ...
                f"ARG ANSIBLE_GALAXY_CLI_COLLECTION_OPTS={self.definition.build_arg_defaults['ANSIBLE_GALAXY_CLI_COLLECTION_OPTS']}",
                "USER root",
                "",
            ]
        )
